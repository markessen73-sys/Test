 ...
 * Pop eyes are animated live on punch (popEyes.ts), not baked here.
 */
function makeOoh(
  img: HTMLImageElement,
  landmarks: NormalizedLandmark[],
  _features?: CustomFaceFeatures,
): string {
  const w = img.naturalWidth || img.width || FACE_OUT_SIZE;
  const h = img.naturalHeight || img.height || FACE_OUT_SIZE;
  const canvas = document.createElement('canvas');
  ...
  // Pop eyes are animated live on punch (see popEyes.ts) — not baked here.
