// @ts-nocheck — clown makeup/wig/KO painters for browser custom-boxer bake
import {
  W, H, LM, clamp, mix, isBackdrop, isIris, isSclera, isTooth, isGlassesFrame,
  isSkinTone, isBlondeHair, ellipseDist, softEdge,
} from './faceDamageBake';
 ...
 */
export function applyComedyClownMakeup(face, clean) {
  let painted = 0;
...
export function paintCurlyClownWig(face, clean) {
  const palette = [
    [255, 45, 55],
    [255, 210, 40],
    [45, 110, 255],
...
export function applyClownKnockout(face, clean) {
