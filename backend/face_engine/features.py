...
from .eyes import EyeFeatures, detect_eyes_and_glasses
...
class FaceFeatures:
    ...
    iris_hex: str = "#5a5a6e"
    iris_sample_count: int = 0
    eye_scale: float = 1.22
...
def _quantize(c: tuple[int, int, int], levels: int = 6) -> tuple[int, int, int]:
    """Snap non-skin / non-iris colours toward flat cartoon palette steps."""
...
def extract_features(bgr: np.ndarray, lm: PhotoLandmarks) -> FaceFeatures:
    ...
    eyes = detect_eyes_and_glasses(bgr, lm)
    ...
    return FaceFeatures(
        ...
        skin_tone=tone,
        iris_hex=eyes.iris_hex,
        iris_sample_count=eyes.iris_sample_count,
        eye_scale=eyes.eye_scale,
        has_glasses=eyes.has_glasses,
